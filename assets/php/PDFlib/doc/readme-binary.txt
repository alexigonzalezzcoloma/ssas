...
This is a binary package containing PDFlib, PDFlib+PDI, and PDFlib
Personalization Server (PPS) in a single binary. It requires a commercial
license to use it. However, the library is fully functional for evaluation
purposes.

Unless a valid license key has been applied the generated PDF output will
have a demo stamp across all pages. See the PDFlib tutorial (chapter 0) to
learn how to apply the license key.

Operating systems requirements for using PDFlib are detailed in
...
The PDFlib header file pdflib.h plus a PDFlib library are contained in the
...
Windows
-------
Windows editions are available in the following flavors for use with
Visual Studio and compatible compilers:

- 32-bit Windows DLL for C/C++
  The DLL pdflib.dll is supplied with the corresponding import library
  pdflib.lib. In order to build and run the supplied C/C++ samples
  copy these files to the bind/c or bind/cpp directories.

- 64-bit Windows DLL for C/C++

- A static library for C/C++ is available upon request.

The Windows libraries are not compatible with Borland/CodeGear/Embarcadero
C++ Builder.
...
IBM AIX
-------
PDFlib for AIX is built with the IBM C compiler (IBM XLC). 

Using PDFlib with gcc/g++ on AIX:
If you want to use PDFlib with C or C++ applications which are built with
gcc/g++: the GNU compiler on AIX by default is not fully
...
the following gcc compiler option when compiling your application:
-mxl-compat
...
===============================
Notes for using PDFlib on IBM Z
===============================
...
CICS applications can use the CEEUOPT mechanism.
