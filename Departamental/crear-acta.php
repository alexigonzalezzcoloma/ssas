...
if (isset($_SESSION['user_rut'])){
   if ($_SESSION["id_rol"] == '2'){
      ...
      if(isset($_POST["postComission"])){
         ...
         if (isset($_POST["checkrequest"])){
            $requestsList=$_POST["checkrequest"];
            $queryLastId="SELECT id_anual, YEAR(fecha_hora) as lastVoteYear FROM `votos_aprobacion` ORDER BY id DESC LIMIT 1;";
            ...
            if($numResults==1){
               ...
               $lastVoteYear = $rowsLastId[1];
               $actualDate= date("Y");
               if ($actualDate == $lastVoteYear){
      ...
      $cieNum=$_POST["cieNum"];
      $pdf->setHeaderDesign($realizedOn,$isRecord,$cieNum);
      ...
      for ($i=0;$i<$num_req;$i++){
         ...
         DATE_FORMAT(solicitudes.fecha_voto_interno , '%d-%m-%Y')fecha_voto_interno, solicitudes.tipo FROM `solicitudes` 
         ...
         $mayusSubs=strtoupper($subsGroup);
         $voteNumberFormated=zero_fill($voteList[$i],3);
         $pdf->Write(5,$str = utf8_decode("$thisyear - $voteNumberFormated  $mayusSchool - $recordTitle - RECURSOS $mayusSubs - $concept[$i]"));
         $pdf->SetFont('Times','',12);
         $pdf->Ln(10);
         ...
         if ($reqType=="Contratos"){
            ...
            if($num_rows_pme>0){
               ...
               $pdf->Cell(40,6,utf8_decode('SUBVENCIÓN'),1,0,'C');
               $pdf->Cell(40,6,utf8_decode('DIMENSIÓN'),1,0,'C');
               $pdf->Cell(40,6,utf8_decode('SUBDIMENSIÓN'),1,0,'C');
               $pdf->Cell(40,6,utf8_decode('ACCIÓN'),1,1,'C');
               $pdf->SetFont('Times','',10);
            ...
            if($num_req>0){
               ...
               $pdf->Ln(6);
               $pdf->Cell(40,6,utf8_decode('NOM. PROFESIONAL'),1,0,'C');
               $pdf->Cell(60,6,utf8_decode('ASIGNATURA'),1,0,'C');
               $pdf->Cell(30,6,utf8_decode('HORAS'),1,0,'C');
               $pdf->Cell(30,6,utf8_decode('SUBVENCIÓN'),1,1,'C');
               $pdf->SetFont('Times','',10);
            ...
            if($isRecord==1){
               $pdf->SetFont('Times','B',10);
               $pdf->Cell(160,6,utf8_decode("ID DE LA SOLICITUD: $requestsList[$i]"),1,1,'C');
            }
         }else{
            $pdf->SetFont('Times','B',10);
            $pdf->Cell(80,6,utf8_decode('Nombre Escuela o Colegio'),1,0,'C');
            ...
            if($num_rows_req>0){
               ...
               $pdf->Ln(6);
               $pdf->Cell(58,6,utf8_decode('DETALLE'),1,0,'C');
               $pdf->Cell(22,6,utf8_decode('VALOR UN.'),1,0,'C');
               $pdf->Cell(22,6,utf8_decode('CANTIDAD'),1,0,'C');
               $pdf->Cell(28,6,utf8_decode('SUBVENCIÓN'),1,0,'C');
               $pdf->Cell(30,6,utf8_decode('VALOR TOTAL'),1,1,'C');
               
               $pdf->SetFont('Times','',10);
               $pdf->SetWidths(Array(58,22,22,28,30));
      ...
      if($isRecord==1){
         ...
         $pdf->Ln(18);
         $pdf->SetFont('Times','B',12);
         $pdf->Write(5,$str = utf8_decode("ORACIÓN: "),0,1,'L'); 
         $pdf->SetFont('Times','',12);
         $pdf->Write(5,$str = utf8_decode("$finalPray "),0,0,'L');
         $pdf->Ln(18);
         $yImage = $pdf->GetY();
         $pdf->Image($PresidentFirm,25,$yImage,60,30);
         $pdf->Image($SecretaryFirm,135,$yImage,60,30);
         $pdf->Text(28, $yImage+36, $str = utf8_decode("$PresidentName, presidente"));
         $pdf->Text(140, $yImage+36, $str = utf8_decode("$SecretaryName, secretario"));
      }
      $pdf->Output();
