...
<body id="page-top">
    <div id="wrapper">
        <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0" style="background-color: rgb(0,50,100);">
            <div class="container-fluid d-flex flex-column p-0">
                ...
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    <li class="nav-item"><a class="nav-link active" href="inicio.php"><i class="fas fa-tachometer-alt"></i><span>Mi Bandeja</span></a></li>
                    ...
                    <li class="nav-item"><a class="nav-link" href="comision-interna.php"><i class="fas fa-comment-slash"></i><span>Comisiones Internas</span></a></li>
        ...
        <div class="d-flex flex-column" id="content-wrapper">
                ...
                <nav class="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top">
                    ...
                    <ul class="nav navbar-nav flex-nowrap ml-auto">
                        <li class="nav-item dropdown no-arrow mx-1">
                            <div class="nav-item dropdown no-arrow">
                                ...
                                <a class="dropdown-toggle nav-link" data-toggle="dropdown" aria-expanded="false" href="#">
                                    <span class="badge badge-danger badge-counter" id="numNotifications"></span>
                                    <i class="fas fa-bell fa-fw"></i>
                                </a>
                                <div class="dropdown-menu dropdown-menu-right dropdown-list dropdown-menu-right animated--grow-in">
                                    <h6 class="dropdown-header">Notificaciones</h6>
                                    <button class="btn button center btn-sm" onclick="deleteAllNotifications()">Borrar Todas</button>
                                    <div id="notificationCenter">

                                    </div>
                                    <!--<a class="text-center dropdown-item small text-gray-500" href="#">Mostrar Todas</a>-->
                                </div>
                            </div>
                        </li>
                        <li class="nav-item dropdown no-arrow mx-1">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <div class="nav-item dropdown no-arrow"><a class="dropdown-toggle nav-link" data-toggle="dropdown" aria-expanded="false" href="#"><span class="d-none d-lg-inline mr-2 text-gray-600 small"><?php echo $userName;?> - Editor</span></a>
            ...
            $sql_states="SELECT estados.id, estados.nombre FROM estados";
            $result_states= mysqli_query($connection,$sql_states);
            $num_states=mysqli_num_rows($result_states);
            ?>
            ...
            <div class="container-fluid" style="margin: 0px;">
                <div class="card shadow">
                    ...
                    <a class="btn btn-danger" href="inicio.php" style="margin: 0px;padding: 0px;height: 29px;width: 103px;">Limpiar</a>
                    <a class="btn btn-success btn-sm" role="button" style="margin: 0px;padding: 0px;height: 29px;width: 103px;" id="btnStartRequest">Crear Solicitud</a>
                    </div>
                    </form>
                    <div class="card-body">
                        <div class="container">
                            <div class="row">
                                <div class="col">
                                    ...
                                    <select class="form-control" id="selectState">
                                        ...
                                        <?php
                                            if($num_states > 0){
                                                while($states=mysqli_fetch_array($result_states)){
                                                    if($state==$states["id"]){
                                        ?>
                                        <option value="<?php echo $states['id'];?>" selected><?php echo $states['nombre'];?></option>
                                        <?php
                                                    }else{
            
                                        ?>
                                        <option value="<?php echo $states['id'];?>"><?php echo $states['nombre'];?></option>
                                        <?php
                                                    } 
                                                }
                                            }
                        ...
                        <div class="table-responsive table-bordered table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info" style="font-size: 11px;height: auto;width: auto;">
                                        <table class="table table-striped table-bordered dataTable my-0" id="dataTable">
                                            ...
                                            <?php
                                                ...
                                                INNER JOIN estados on solicitudes.id_estado_actual=estados.id where solicitudes.rbd_colegio='$schoolRbd'";
                                                
                                                if (isset($_REQUEST['busqueda']) && isset($_REQUEST['estado'])){
                                                    $search=$_REQUEST["busqueda"];
                                                    $state=$_REQUEST["estado"];
                                                    if($state==""){
                                                        $what_search='busqueda='.$search;
                                                        $sql1=$sql1." and solicitudes.titulo LIKE '%$search%' or solicitudes.id='$search'";
                                                    }else{
                                                        $what_search='busqueda='.$search.'&estado='.$state;
                                                        $sql1=$sql1." and solicitudes.id_estado_actual='$state' and solicitudes.titulo LIKE '%$search%' or solicitudes.id_estado_actual='$state' and solicitudes.id='$search'";
                                                    }
                                                }
                                                if (isset($_REQUEST['estado']) && !isset($_REQUEST['busqueda'])){
                                                    $state=$_REQUEST["estado"];
                                                    if($state==""){
                                                        $what_search='';
                                                        $sql1=$sql1;
                                                    }else{
                                                        $what_search='estado='.$state;
                                                        $sql1=$sql1." and solicitudes.id_estado_actual='$state' ";
                                                    }
                                                }
                                                if (!isset($_REQUEST['estado']) && isset($_REQUEST['busqueda'])){
                                                    $search=$_REQUEST["busqueda"];
                                                ...
                                                if($num_rows_sql2>0){
                                                    while ($row=mysqli_fetch_array($result2)){
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><a class="btn btn-warning btn-circle ml-1" role="button" style="font-size: 11px;width: 30px;height: 30px;margin: -8px;" href="solicitud.php?numero=<?php echo $row["id"];?>"><i class="fas fa-edit text-white"></i></a></td>
                                                        </tr>
                                        ...
                                        <nav class="d-lg-flex justify-content-lg-end dataTables_paginate paging_simple_numbers my-2">
                                            <ul class="pagination">
                                                ...
                                                ?>
                                                <li class="page-item"><a class="page-link" href="?pagina=<?php echo 1; ?>&<?php echo $what_search; ?>"><span aria-hidden="true">|<</span></a></li>                            
                                                <li class="page-item"><a class="page-link" href="?pagina=<?php echo $pagina-1; ?>&<?php echo $what_search;?>" aria-label="Previous"><span aria-hidden="true"><<</span></a></li>                            
                                               <?php
                                                    }
                                                    for($i=1; $i<=$total_pages; $i++){
                                                        ...
                                                        if($i==$pagina){
                                                            ...
                                                            echo '<li class="page-item"><a class="page-link" href="?pagina='.$i.'&'.$what_search.'">'.$i.'</a></li>';
                                                        }    
                                                    }
                                                    if($pagina != $total_pages){
                                                ?>      
                                                <li class="page-item"><a class="page-link" href="?pagina=<?php echo $pagina+1;?>&<?php echo $what_search;?>"aria-label="Next"><span aria-hidden="true">>></span></a></li>
                                                <li class="page-item"><a class="page-link" href="?pagina=<?php echo $total_pages;?>&<?php echo $what_search;?>"><span aria-hidden="true">>|</span></a></li>
                                                <?php }?>
                                            </ul>
                                        </nav>
                                        <?php
        ...
        <footer class="bg-white sticky-footer">
            ...
            <div class="modal fade" id="createRequest" role="dialog" tabindex="-1">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title">Crear Solicitud</h4><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button></div>
                        <div class="modal-body">
                        ...
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-primary" type="button" id="btnCreateRequest">Crear Solicitud</button>
                        </div>
                    </div>
                </div>
            </div>
