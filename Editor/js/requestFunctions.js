                ...
                $.ajax({
                    ...
                    success: function(response){	
                        if(response==1){
                            ...
                            alertify.error("Error al guardar encabezado, revise que el título no sea muy extenso");
                            console.log(response);
                        }  
                    }
                });  
    ...
    if(type=="Servicios"){
        ...
        if(serviceName!=""){
            if(serviceQuantity!="" && isNaN(serviceQuantity)==false){
                if(servicePrice!="" && isNaN(servicePrice)==false){
                    if(serviceSubs!=""){
...
function addAttachment(requestId){
    ...
    if(attachFile.length != 0){
        ...
        if ((/\.(pdf)$/i).test(File.name)){
            ...
            alertify.success("Se está el Archivo, espere hasta que la ventana se cierre automáticamente");            
            $.ajax({
                type:"POST",
...
function sendRequest(requestId){
    alertify.confirm('Eliminar', '¿Esta segur@ de enviar la solicitud?', 
	    function(){ 
            $.ajax({
...
function sendSpecialRequest(requestId){
    totalPrice=$('#totalPrice').val();
    totalPrice=cleanChar(totalPrice,'.');
    requestType=$('#requestType').val();
    subId=$('#subSpecialReq').val();
    data1="requestId="+requestId+"&totalPrice="+totalPrice+"&subId="+subId;
    if(requestType=="Contratos"){
        if(totalPrice!=""){
            $.ajax({
                url: 'php/sendSpecialRequest.php',
                type: 'POST',
                data:data1,
                success: function(response){
                    if (response==1){
            ...
            alertify.error("Debe ingresar el total monetario en la sección correspondiente");
        }     
    }else{
        alertify.error("La solicitud solo es compatible con Contratos");
    } 
}
    ...
    if(id!=""){
        if(subvention!=""){
            if(dimension!=""){
                if(subdimension!=""){
                    if(action!=""){
                        cadena="id="+id+"&subvention="+subvention+"&dimension="+dimension+"&subdimension="+subdimension+"&action="+action+"&requestId="+requestId;
                        console.log(cadena);
                        $.ajax({
